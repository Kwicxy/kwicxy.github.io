...
description: 梳理函数单调性、极值、最值、凹凸性、拐点和图形描绘的判别方法，强调定义域、端点与不可导点等易错条件。
math: true
mermaid: true
...
tags:
---
> 梳理函数单调性、极值、最值、凹凸性、拐点和图形描绘的判别方法，强调定义域、端点与不可导点等易错条件。
...
+++ 例题 5.1：由导数符号判定单调区间
求函数 $f(x)=x^2e^{-x}$ 的单调区间。

**【解】** 函数的定义域为 $\mathbb R$，且
...
+++ 例题 5.5：二阶判别失效时使用高阶导数
分别判断 $f(x)=x^4$、$g(x)=x^5$ 在 $x=0$ 处是否取得极值。
...
+++ 例题 5.6：闭区间上的最值
求函数 $f(x)=x^3-3x$ 在闭区间 $[-2,3]$ 上的最大值和最小值。
...
+++ 例题 5.8：二阶导数为零不等于拐点
比较曲线 $y_1=x^3$、$y_2=x^4$ 在原点处的性质。

**【解】** 两者都满足 $y_1''(0)=y_2''(0)=0$，但
...
+++ 例题 5.11：使用高阶导数判别拐点
分别判断 $f(x)=x^5+1$、$g(x)=x^6+1$ 在 $x=0$ 处是否有拐点。
...
+++ 例题 5.12：由零点重数判断局部图像性质
设 $f(x)=(x+2)^3(x-1)^4(x-4)$。只利用零点重数判断各零点处能够直接确定的图像性质。
...
+++ 例题 5.13：由实根重数计算极值点和拐点个数
设 $f(x)=(x-1)^2(x-2)^3(x-3)^4(x-4)^5(x-5)$，求 $f$ 的极值点和拐点个数。

**【解】** 各零点的重数依次为 $2,3,4,5,1$，因此
...
+++ 例题 5.14：求不同方向上的渐近线
求函数 $f(x)=\dfrac1x+\ln(1+e^x)$ 的全部渐近线。
...
+++ 例题 5.15：闭区间上的最值
求 $f(x)=x+\dfrac4x$ 在 $[1,5]$ 上的最大值和最小值。
...
+++ 例题 5.16：开区间上的最值与取值范围
求函数 $f(x)=x^2-2x$ 在 $(0,3)$ 上的最值与取值范围。
...
+++ 例题 5.17：参数曲线的曲率与曲率半径
求曲线 $x=\cos^3t$、$y=\sin^3t$ 在 $t=\dfrac{\pi}{4}$ 对应点处的曲率和曲率半径。

**【解】** 当 $t=\dfrac{\pi}{4}$ 时，对应点为
...
+++ 例题 5.18：隐式曲线的作图
作出曲线 $y^2=(1-x^2)^3$ 的草图。

**【解】** 由 $y^2\ge0$ 可知必须有 $1-x^2\ge0$，故
...
分别在极坐标系和直角坐标系中作出曲线 $r=\sin^2\theta$ 的草图。
