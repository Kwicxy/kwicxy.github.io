...
description: 系统整理一元函数求导公式与计算法则，涵盖复合、隐函数、参数方程和高阶导数的求导方法。
math: true
...
tags:
---
> 系统整理一元函数求导公式与计算法则，涵盖复合、隐函数、参数方程和高阶导数的求导方法。
...
**【解】** 函数由 $x^2$、$\sin(x^2)$、$e^{\sin(x^2)}$ 三层复合而成，因此
...
**【解】** 由于 $\sqrt{1+x^2}>|x|$，对任意 $x\in\mathbb R$ 都有
...
+++ 例题 4.8：由反函数公式推导反三角函数导数
令 $y=\arcsin x$、$x=\sin y$，其中 $y\in\left[-\dfrac\pi2,\dfrac\pi2\right]$。
...
+++ 例题 4.13：多因子乘除
设 $x>1$，求 $y=\dfrac{(x^2+1)^3\sqrt{x-1}}{e^x}$ 的导数。
...
+++ 例题 4.14：绝对值对数的使用
在 $x\ne\pm1$ 的区间内，求 $y=\dfrac{(x-1)^2}{(x+1)^3}$ 的导数。
...
+++ 例题 4.16：三角型幂指函数
在 $\sin x>0$ 的区间内，求 $y=(\sin x)^{\cos x}$ 的导数。
...
+++ 例题 4.18：三角函数的循环
求 $f(x)=\sin(3x)$ 的第 $n$ 阶导数。

**【解】** 每求一次导数，相位增加 $\pi/2$，同时由链式法则产生一个因子 $3$，故
...
+++ 例题 4.19：多项式与指数函数的乘积
求 $f(x)=x^2e^{3x}$ 的 $n$ 阶导数。

**【解】** 因为 $(x^2)^{(k)}=0$（$k\ge3$），莱布尼茨公式只保留 $k=0,1,2$：
...
+++ 例题 4.20：在指定点计算高阶导数
设 $f(x)=x^3e^x$，求 $f^{(n)}(0)$。

**【解】** 由莱布尼茨公式，$x^3$ 只有三阶导数在 $x=0$ 时非零：
